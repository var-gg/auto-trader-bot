...
def run_fold_validation(*, request: RunnerRequest, data_path: str | None, data_source: str, scenario_id: str | None, strategy_mode: str, runner_fn: Callable[..., dict], holding_overlap: float = 1.0, mode: str = "walk_forward", max_folds: int | None = None, summary_only: bool = False, diagnostics_lite: bool = False, emit_timing_logs: bool = False, bootstrap_result: dict | None = None) -> dict:
    ...
    bootstrap_candidate_reuse = (bootstrap.get("artifacts") or {}).get("candidate_reuse") or {
        ...
        "metadata": {
            ...
            "macro_series_history": bootstrap.get("macro_series_history") or bootstrap_ctx.get("macro_series_history") or [],
            "sector_map": bootstrap.get("sector_map") or bootstrap_ctx.get("sector_map") or {},
            "session_metadata_by_symbol": bootstrap.get("session_metadata_by_symbol") or bootstrap_ctx.get("session_metadata_by_symbol") or {},
        },
        "session_metadata_by_symbol": bootstrap.get("session_metadata_by_symbol") or bootstrap_ctx.get("session_metadata_by_symbol") or {},
        "macro_series_history": bootstrap.get("macro_series_history") or bootstrap_ctx.get("macro_series_history") or [],
    ...
    for idx, split in enumerate(normalized, start=1):
        ...
        train_ctx = train_result.get("historical_context") or train_result.get("artifacts", {}).get("historical_context") or {}
        test_ctx = test_result.get("historical_context") or test_result.get("artifacts", {}).get("historical_context") or {}
        ...
        train_metrics = compute_performance_metrics(plans=[_dict_to_plan(p) for p in train_result.get("plans") or []], fills=[_dict_to_fill(f) for f in train_result.get("fills") or []], bars_by_symbol=train_ctx.get("bars_by_symbol") or bootstrap_ctx.get("bars_by_symbol") or bootstrap.get("bars_by_symbol") or {}, total_symbols=len(request.scenario.symbols)) if train_result.get("plans") is not None else {}

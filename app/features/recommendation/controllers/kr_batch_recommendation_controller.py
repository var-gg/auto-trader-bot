# app/features/recommendation/controllers/kr_batch_recommendation_controller.py
...
from app.features.recommendation.services.kr_batch_recommendation_service import KrBatchRecommendationService
...
@router.post(
    ...
    description="""
    추천 후보 티커들에 대해 배치로 AI 추천을 생성합니다 (국내주식, asyncio 기반).
    
    **처리 과정:**
    1. 휴장 여부 확인: `/marketdata/kr/is-market-closed` 서비스 활용
    2. 후보 티커 조회: `/recommendations/kr/candidates` 서비스 활용 (days_back=1 고정)
    2.5. 마켓데이터 동기화: `/marketdata/kr/sync/daily` 서비스 활용 (days=1 고정)
    ...
    **국내주식 특화:**
    - 국내주식 전용 휴장 확인 및 마켓데이터 동기화
    - 국내주식 후보 티커 조회
    - 국내주식 AI 분석 서비스 활용
    
    **사용 예시:**
    - `POST /recommendations/kr/batch-generate` (인자 없음)
    """,
    response_description="배치 처리 결과와 생성된 추천 목록 (국내주식)"
)
async def generate_batch_recommendations_async(
    db: Session = Depends(get_db)
):
    """배치 추천 생성을 실행합니다 (국내주식, 비동기)."""
    try:
        service = KrBatchRecommendationService(db)

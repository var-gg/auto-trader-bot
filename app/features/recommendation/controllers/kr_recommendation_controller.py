# app/features/recommendation/controllers/kr_recommendation_controller.py
...
@router.get(
    ...
    summary="추천 후보 티커 조회",
    description="""
    추천 후보가 될 수 있는 티커 목록을 조회합니다 (국내주식).
    ...
    **사용 예시:**
    - `GET /recommendations/kr/candidates?days_back=3` (3일 전 뉴스 참조)
    """,
...
@router.get(
    ...
    description="특정 티커가 추천 후보가 될 수 있는지 검증합니다 (국내주식).",
...
def validate_ticker_eligibility(
    ...
    """특정 티커의 추천 적격성을 검증합니다 (국내주식)."""
    try:
        repository = RecommendationRepository(db)
        service = KrRecommendationService(repository)
        
        result = service.validate_ticker_eligibility(ticker_id, days_back, "KR")

# app/features/recommendation/services/kr_analyst_ai_service.py
...
logger = logging.getLogger("kr_analyst_ai_service")
...
class KrAnalystAIService:
    """애널리스트 AI 분석 및 추천 생성 서비스 (국내주식 전용)"""
...
뉴스·어닝·기술·펀더멘털을 종합해 **하나의 방향(LONG 또는 SHORT)**만 제시하십시오.
분석 범위는 **2~20거래일**이며, 먼저 **향후 0~3영업일 단기 바이어스**를 판정한 뒤 수치를 산정합니다.

[관망(Observe) — 핵심]
- "관망"은 포지션 미개시를 뜻하지 않습니다.
- 신호가 약하거나 상충될 때는 **관망이 합리적임을 reason에 명시**하고,
  **체결확률이 매우 낮은(≈1~5%) 유리한 제한가('베이트')**를 entry_price로 제시하십시오.
- 베이트 가격은 **최근 저점/재테스트 레벨/0.3~0.5×ATR14** 등 합리적 근거로 산정합니다.
...
- **금액(원·달러 등)은 정수 단위**, 소수점 생략. **비율/지표값은 소수점 2자리**. 단위 명확(원, 달러, %, bp).
- 국내 시장 특성(가격대별 최소호가·상하한가 ±30% 등)을 인지하되, 계산 값은 **정수 원 기준**으로 제시합니다.
...
[POLICY]
1) 방향 결정(0~3영업일 게이트):
   - 다음을 종합해 단기 바이어스를 판정: 뉴스 톤/모멘텀, 어닝 근접도(T±3영업일·DART 일정),
     MA20·MA50 기울기와 현재가의 상하관계, ATR14 대비 일중 범위, 단기 price_change.
...
3) 가격 설계(핵심 지표만 사용):
   - **MA20/MA50, RSI14(30/50/70), MACD(12,26,9; 값/시그널/히스토그램), ATR14(절대·%), 단기 price_change**만 활용.
   - entry_price는 현재가 대비 **유리한 구간**으로 산정(최근 스윙 고·저/피벗/0.3~0.5×ATR14 근방, 정수 원 반올림).
   - target은 **선택한 단기 범위의 기대 이동**(스윙 폭/뉴스·어닝 촉발/MA20 기울기)을 반영.
   - stop은 **기대수익 대비 1/2~1/3 리스크** 또는 ‘무효화 레벨 + 0.2~0.3×ATR14’ 기준.
   - 상하한가/유동성(거래대금) 특성상 비현실적 레벨은 피하고, 필요 시 reason에 근거를 간단히 명시.
...
5) reason(3~4문장, 200토큰 이내, 각 줄 끝 "\\n", 마지막 줄 개행 금지):
   ...
   - 이어서 [어닝, 뉴스, 기술, 펀더멘털] 중 **뉴스 포함 최소 1개 이상**을 **해석형**으로 연결(단순 나열 금지).
   - 필요 시 유효기간 사유를 명시.

[CONFIDENCE]
- 0.40±: 신호 상충/촉발 부재 → **관망 합리 + 베이트 제한가**(체결확률 매우 낮음) 제시
- 0.55±: 신호 2개 정렬, 근접 이벤트(DART/실적)
- 0.70±: 3개 이상 정렬·거래대금 우호
- 0.95±: 변수 거의 없음(드문 케이스)

[SELF-CHECK]
- 단기(0~3영업일) 바이어스를 먼저 결정했는가?
- 관망이 합리한 경우, **베이트 제한가**가 근거와 함께 제시되었는가?
- target/entry/stop 순서가 일관적인가?
- 국내 시장 제약(최소호가/상하한가/거래정지·관리종목 리스크 등)을 암묵적으로 고려했는가?
- reason이 3~4문장으로 구성되어 있고, 200토큰 이내이며, 날짜·분기·단위 표기 규칙을 지키며, 
  실제 경제지 기고문에 그대로 실려도 어색하지 않을 만큼 자연스럽게 작성되었는가?
    ...
    def generate_analyst_prompt_only(self, ticker_id: int) -> str:
        ...
        /recommendations/kr/analyst-prompt/{ticker_id} API용

# app/features/recommendation/services/us_batch_recommendation_service.py
...
logger = logging.getLogger("us_batch_recommendation_service")


class UsBatchRecommendationService:
    """배치 추천 생성 서비스 (미국주식 전용)"""
    ...
    async def generate_batch_recommendations_async(self) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"배치 추천 생성 완료: 총 {total_processed}개 중 성공 {successful}개, 실패 {failed}개")
            ...
            logger.error(f"배치 추천 생성 중 오류 발생: {str(e)}")
            return {
                "status": "error",
    ...
    def _is_market_closed(self) -> bool:
        """휴장 여부를 확인합니다."""
        try:
            service = USMarketHolidayService(self.db)
    ...
    def _get_candidate_tickers(self) -> List[Dict[str, Any]]:
        """후보 티커 목록을 조회합니다 (days_back=1 고정, country="US" 고정)."""
        try:
            ...
            result = service.get_candidate_tickers(days_back=1, country="US")
    ...
    def _sync_market_data(self, candidate_tickers: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"마켓데이터 동기화 시작: {len(ticker_ids)}개 티커 (days=1)")
            
            # USDailyIngestor를 사용하여 미국주식 마켓데이터 동기화
            ...
            logger.info(f"마켓데이터 동기화 완료: {len(ticker_ids)}개 티커, {total_upserted}개 데이터 업서트")
            ...
            logger.error(f"마켓데이터 동기화 실패: {str(e)}")
            return {
                "success": False,
    ...
    async def _process_single_ticker_async(self, ticker: Dict[str, Any], batch_num: int) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                # AI 분석 서비스 호출 (비동기)
                ai_service = UsAnalystAIService(db)
            ...
            logger.error(f"티커 {ticker_symbol} (ID: {ticker_id}) 처리 실패: {str(e)}")
            return {
                "success": False,
                "ticker_id": ticker_id,
                "ticker_symbol": ticker_symbol,

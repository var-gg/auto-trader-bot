# app/features/signals/models/intraday_signal_detection_config.py
...
class IntradaySignalDetectionConfig(Base):
    """
    분봉 시그널 탐지 설정 테이블
    ...
    - 5분봉 기반 시그널 탐지용
    """
    __tablename__ = "intraday_signal_detection_config"
    __table_args__ = (
        UniqueConstraint(
            "direction", "lookback", "future_window", 
            "min_change", "max_reverse", "flatness_k", "atr_window", "version",
            name="uq_intraday_config_params"
        ),
        CheckConstraint("direction IN ('UP', 'DOWN')", name="ck_intraday_direction"),
        {"schema": DB_SCHEMA},
    )

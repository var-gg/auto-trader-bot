# app/features/portfolio/services/domestic_fill_collection_service.py
...
from app.features.kis_test.models.kis_test_models import KISDomesticOrderRequest, KISDomesticOrderResponse

logger = logging.getLogger(__name__)


class DomesticFillCollectionService:
    """국내주식 체결정보 수집 서비스
    ...
    async def collect_domestic_fills(self, days_back: int = 7) -> Dict[str, Any]:
        """국내주식 체결정보 수집 (일별로 소분해서 조회)
        ...
        """
        try:
            logger.info(f"국내주식 체결정보 수집 시작 - {days_back}일간 (일별 조회)")
            ...
            for day_offset in range(days_back):
                ...
                # 1. 체결 조회 (CCLD_DVSN="01")
                filled_result = await self._collect_domestic_fills_single_day(date_str, "01")
                ...
                # 2. 미체결 조회 (CCLD_DVSN="02")
                unfilled_result = await self._collect_domestic_fills_single_day(date_str, "02")
            ...
            logger.info(f"국내주식 체결정보 수집 완료 - {days_back}일간, 처리: {total_processed}건, 업서트: {total_upserted}건, 스킵: {total_skipped}건")
            ...
            logger.error(f"국내주식 체결정보 수집 중 오류: {str(e)}")
            self.db.rollback()
            return {
                "success": False,
                "error": str(e),
    ...
    async def _collect_domestic_fills_single_day(self, date_str: str, ccld_dvsn: str = "00") -> Dict[str, Any]:
        """하루치 국내주식 체결정보 수집
        ...
        try:
            ...
            result = self.kis_client.domestic_order_test(
                CANO="00000000",
                ACNT_PRDT_CD="01",
                INQR_STRT_DT=date_str,  # 하루만
                INQR_END_DT=date_str,    # 하루만
            ...
            # 체결정보 처리
            output_data = result.get("output1", [])
            if not output_data:
                return {
                    "success": True,
                    "processed_count": 0,
            ...
            logger.debug(f"{date_str} (체결구분: {ccld_dvsn}): {len(output_data)}건")
    ...
    async def _process_order_fill(self, order_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not broker_order:
                logger.debug(f"broker_order 없음 (스킵) - order_number: {order_number}, pdno: {order_data.get('pdno')}, prdt_name: {order_data.get('prdt_name')}")
                return False
            
            # 체결정보 추출
            fill_qty = self._parse_int(order_data.get("tot_ccld_qty", "0"))
            fill_price = self._parse_decimal(order_data.get("avg_prvs", "0"))
    ...
    def _determine_fill_status(self, order_data: Dict[str, Any], broker_order: BrokerOrder) -> str:
        ...
        """
        # 매매구분명에서 취소 확인 (예: "매수취소*", "매도취소*")
        sll_buy_dvsn_cd_name = order_data.get("sll_buy_dvsn_cd_name", "")
        if "취소" in sll_buy_dvsn_cd_name:
            return "CANCELLED"
        
        # 취소여부 확인
        cncl_yn = order_data.get("cncl_yn", "")
        if cncl_yn == "Y":
            return "CANCELLED"
        
        # 거부수량 확인
        rjct_qty = self._parse_int(order_data.get("rjct_qty", "0"))
        order_qty = self._parse_int(order_data.get("ord_qty", "0"))
        
        # 전체 수량이 거부된 경우
        if rjct_qty > 0 and rjct_qty == order_qty:
            return "REJECTED"
        ...
        if fill_qty == 0:
            # 거부 수량이 일부만 있는 경우도 UNFILLED로 처리
            return "UNFILLED"

...
class BuyOrderAIService:
    """매수주문 AI 생성 서비스"""
    
    def __init__(self, db: Session):
        self.db = db
        self.prompt_service = BuyOrderPromptService(db)
    ...
    def generate_buy_order_batch(self) -> Dict[str, Any]:
        """
        매수주문 배치를 AI로 생성하고 즉시 실행합니다.
        
        Returns:
            Dict[str, Any]: 생성된 주문 배치 정보 및 실행 결과
        """
        logger = logging.getLogger(__name__)
        logger.info("=== 매수주문 AI 배치 생성 시작 ===")
        ...
        try:
            ...
            logger.info(f"🎉 매수주문 AI 배치 생성 및 실행 완료: 배치 ID {batch_id}")
            return result
            
        except Exception as e:
            logger.error(f"❌ 매수주문 AI 배치 생성 중 오류: {str(e)}")
            raise
    
    def _call_buy_order_ai(self, prompt_text: str) -> Dict[str, Any]:
        """매수주문 AI를 호출하여 주문 배치를 생성합니다."""
        logger = logging.getLogger(__name__)

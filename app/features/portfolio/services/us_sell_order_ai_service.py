...
class SellOrderAIService:
    """매도주문 AI 생성 서비스"""
    
    def __init__(self, db: Session):
        self.db = db
        self.prompt_service = SellOrderPromptService(db)
        self.portfolio_service = PortfolioService(db)
        self.market_holiday_service = USMarketHolidayService(db)
    ...
    def generate_sell_order_batch(self) -> Dict[str, Any]:
        """
        매도주문 배치를 AI로 생성하고 즉시 실행합니다.
        
        Returns:
            Dict[str, Any]: 생성된 주문 배치 정보 및 실행 결과
        """
        logger = logging.getLogger(__name__)
        logger.info("=== 매도주문 AI 배치 생성 시작 ===")
        ...
        try:
            ...
            # 1. 자산 스냅샷 강제 갱신 (OVRS)
            logger.info("1️⃣ 자산 스냅샷 강제 갱신 중 (OVRS)...")
            try:
                # AssetSnapshotService를 통해 해외 자산 스냅샷 수집
                ...
                # 해외 계좌 스냅샷 수집
                ovrs_result = asset_snapshot_service.collect_ovrs_account_snapshot()
                
                if "snapshot_id" in ovrs_result:
                    logger.info(f"✅ 해외 자산 스냅샷 갱신 성공 (스냅샷 ID: {ovrs_result['snapshot_id']})")
                else:
                    logger.warning(f"⚠️ 해외 자산 스냅샷 갱신 실패: {ovrs_result}")
                    
            except Exception as e:
                logger.error(f"❌ 해외 자산 스냅샷 갱신 중 오류: {str(e)}")
                # 갱신 실패해도 기존 데이터로 진행
            ...
            result = {
                ...
                "generated_at": datetime.now(timezone.utc).isoformat(),
                "message": f"매도주문 배치 생성 및 실행 완료: 성공 {success_count}개, 실패 {failed_count}개"
            }
            
            logger.info("🎉 매도주문 AI 배치 생성 및 실행 완료")
            return result
            
        except Exception as e:
            logger.error(f"❌ 매도주문 배치 생성 중 오류: {str(e)}")
            raise
    
    def _call_sell_order_ai(self, prompt_text: str) -> Dict[str, Any]:
        """GPT AI를 호출하여 매도주문 배치를 생성합니다."""
        logger = logging.getLogger(__name__)
        ...
        try:
            ...
            response = responses_json(
                ...
                schema_name="sell_order_batch",
                schema=ORDER_BATCH_SCHEMA,
                user_text=prompt_text,
                temperature=0.0,
                task="sell_order_batch"
            )
    ...
    def _execute_order_batch(self, batch_id: int) -> List[Dict[str, Any]]:
        ...
        for plan in execute_plans:
            try:
                # 티커 정보 조회 (exchange 정보 필요)
                ticker = self.db.query(Ticker).filter(Ticker.id == plan.ticker_id).first()
                if not ticker:
                    logger.error(f"❌ 티커 정보를 찾을 수 없습니다: ticker_id={plan.ticker_id}")
                    continue
                
                # 거래소 코드 매핑 (Finnhub → KIS)
                exchange_mapping = {
                    "NMS": "NASD",  # NASDAQ
                    "NYQ": "NYSE",  # NYSE
                    "BTS": "NYSE",  # BATS (NYSE로 매핑)
                }
                exchange = exchange_mapping.get(ticker.exchange, "NASD")
                ...
                for leg in plan.legs:
                    try:
                        # OrderLeg → KIS 주문 파라미터 변환
                        order_params = self._convert_leg_to_order_params(leg, plan, exchange)
                        ...
                        order_result = portfolio_service.order_stock(
                            order_type=order_params["order_type"],
                            symbol=order_params["symbol"],
                            quantity=order_params["quantity"],
                            price=order_params["price"],
                            order_method=order_params["order_method"],
                            exchange=order_params["exchange"],
                            leg_id=leg.id  # 중요: leg_id 연결
                        )
                        ...
                        execution_result = {
                            ...
                            "kis_result": order_result,
                            "status": "SUCCESS" if order_result.get("rt_cd") == "0" else "FAILED",
                            "order_number": order_result.get("ODNO", ""),
                            "message": order_result.get("msg1", "")
                        }
                        ...
                        if execution_result["status"] == "SUCCESS":
                            ...
                            logger.error(f"   🚫 오류: {execution_result['message']} (코드: {order_result.get('msg_cd', 'N/A')})")
    ...
    def _convert_leg_to_order_params(self, leg: "OrderLeg", plan: "OrderPlan", exchange: str) -> Dict[str, Any]:
        """
        OrderLeg를 KIS 주문 파라미터로 변환합니다.
        ...
        order_type = side_mapping.get(leg.side, leg.side.lower())
        
        # 안전한 type 변환 (LIMIT/MARKET/LOC는 그대로 유지)
        valid_methods = ["LIMIT", "MARKET", "LOC"]
        order_method = leg.type if leg.type in valid_methods else "LIMIT"
        
        # 파라미터 검증 로깅
        logger.debug(f"🔄 변환: {leg.side} → {order_type}, {leg.type} → {order_method}")
        
        # 가격 설정 (MARKET 주문은 None이 아닌 "0"으로)
        if order_method == "MARKET":
            price = "0"  # 시장가는 반드시 "0"
        else:
            price = str(leg.limit_price) if leg.limit_price else None

...
class KrSellOrderPromptService:
    """국내주식 매도주문생성용 프롬프트 서비스"""
    ...
    def generate_sell_order_prompt(self) -> str:
        ...
        try:
            logger.info("🚀 국내주식 매도주문 프롬프트 생성 시작")
            ...
            logger.info("✅ 국내주식 매도주문 프롬프트 생성 완료")
            return full_prompt
            
        except Exception as e:
            logger.error(f"❌ 국내주식 매도주문 프롬프트 생성 실패: {str(e)}")
            raise
    ...
    def _get_latest_portfolio_data(self, long_recommendations_by_ticker: Dict[int, Dict[str, Any]], short_ticker_ids: set) -> Dict[str, Any]:
        ...
        logger.info(f"📊 최신 KR 스냅샷 ID: {latest_snapshot.snapshot_id}")
        ...
        for position in latest_snapshot.positions:
            ...
            position_data = {
                ...
                "currency": position.position_ccy or "KRW",
        ...
        logger.info(f"  💵 현금: {account_info['cash_balance']}KRW, 매수가능: {account_info['buying_power']}KRW")
...
# [의사결정 우선순위 — 반드시 이 순서]
1) **손절(필수 조건부)**  2) **희망(필수, 상시·분할)**  3) **익절(선택)**
- **익절은 마지막 수단**이며, **현금 목표/과대비중 해소**가 필요한 경우에만 사용
- **단주 익절 금지**(아래 가드레일 준수)

# [국내 시장 규칙]
- **호가단위**: 1/5/10/50/100/500/1000원 → **항상 호가단위로 내림**
- **일일 상·하한(±30%)** 내에서만 가격 제시
- **동시호가(장전/장후)**: 예상체결가 대비 **유리한 쪽 +1틱 이상**
- **VI 발동**: 현실 레그 비중 확대(체결성 우선), 희망 가격은 유지
...
- **노멀:** 보유수량의 **15~35%**  
- **스트레스:** **25~50%**  
- **익절(현실) 총합 < 10%**인 “단주 익절”은 **금지**(스트레스일 때만 5~10% 허용)  
- orderable이 작아 하한 미달이면 사유 명시

# [레그 구성 — 가격 앵커 & 분할 규칙]
공통 앵커: **target_price / stop_price / current_price**, 통화: **KRW/주**

① **손절 레그（LIMIT, 스트레스에서 우선）**
- 가격: `max(stop_price, current_price - 1틱)` → **호가단위로 내림**, 상하한 내
- 수량: **계획 매도수량의 10~20%**
...
- **가격(결정적)**: `target_price × (1 + u)`에서 u ∈ {**+1%**, **+2%**, **+3%**},  
  상한: `target_price × 1.05`  
  - 즉시체결 방지: 각 희망 가격은 `current_price × 1.005` 이상  
  - **호가단위로 내림**, **상하한 내**

③ **익절(현실) 레그 — 선택**
- 조건: **현금 목표 미달** 또는 **과대비중 해소 필요**
- 가격: `min(target_price, current_price × m)`  
  - **노멀 m=1.006**, **스트레스 m=1.003**  
  - 현재가보다 **+1틱 이상**(즉시체결 방지), **호가단위로 내림**
- 수량: **0~30%**(필요분만)
...
# [우선순위 적용 예]
- stop 근접 & 과대비중 → **손절 15% LIMIT / 희망 분할 60% / (필요시) 익절 15%**
- 현금 충분 & 손실 아님 → **희망 분할 100%**, 익절 생략

# [메모]
- plan.note(2줄): 예) "과대비중·현금 18%: 손절 15% / 희망 target+1·2·3% 분할 60% / 익절 15%"
- batch.notes: 현금목표(20/25/30/35%), 희망 총비중, 익절 총비중, 과대비중 해소 여부

# [SELF-CHECK]
- **KRW/주**, **호가단위 내림**, **상하한 내**
- 수량 정수, 합계 ≤ orderable_quantity
- 가격 범위: **[stop_price ~ target_price+5%]**
- **희망 총수량(노멀) ≥ 50%**(스트레스 20~30%)
- **익절 총합 < 10% ⇒ 생략**(스트레스 예외 5~10%)
- note는 모두 **한국어**
"""

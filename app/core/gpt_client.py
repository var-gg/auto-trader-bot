...
# --- NEW: 모델별 temperature 지원 여부 ---
def _supports_temperature(model: str) -> bool:
    """
    일부 모델(예: gpt-5-mini, gpt-5-nano)은 temperature를 명시적으로 보내면 400을 냅니다.
    이런 모델에 대해선 temperature 파라미터 자체를 생략해야 합니다.
    gpt-5-nano는 오직 temperature=1.0(기본값)만 허용합니다.
    """
    m = (model or "").lower()
    # 알려진 비지원(또는 default만 허용) 패밀리
    no_temp_prefixes = ("gpt-5-mini", "gpt-5-nano", "gpt-nano")
...
def responses_json(
    *,
    ...
    """
    모든 GPT 호출은 여기로 들어오며, 내부에서 자동 로깅(AOP) 수행.
    - 모델이 temperature 비지원이면 해당 파라미터를 제거
    - 400(unsupported_value) 발생 시 temperature 제거 후 재시도
    """
    start = time.time()
    raw_text = None
    tokens_in = None

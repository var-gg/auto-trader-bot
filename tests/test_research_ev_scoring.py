...
from backtest_app.research.scoring import CalibrationModel, EVConfig, apply_calibration_to_test, estimate_expected_value, fit_calibration, fit_calibration_on_fold
...
def _proto(pid, emb, *, side="BUY", regime="RISK_ON", sector="TECH", ret=0.05, win=0.7, mae=-0.01, mfe=0.06, unc=0.01, disp=0.02, supp=3, decayed=2.5, fresh=5):
    return PrototypeAnchor(
        ...
        representative_date="2026-01-01",
        anchor_quality=0.8,
        regime_code=regime,
        sector_code=sector,
        liquidity_score=0.9,
        support_count=supp,
        decayed_support=decayed,
        mean_return_pct=ret,
        median_return_pct=ret,
        win_rate=win,
        mae_mean_pct=mae,
        mfe_mean_pct=mfe,
...
def test_estimate_expected_value_prefers_higher_ev_side():
    ...
    assert long_ev.diagnostics["ev_lift"] == long_ev.calibrated_ev - long_ev.diagnostics["raw_ev"]
    assert long_ev.effective_sample_size >= 1.0
    assert long_ev.top_matches
...
def test_estimate_expected_value_abstains_on_low_ev_or_low_neff():
    ...
    assert "low_ev" in ev.abstain_reasons or "high_uncertainty" in ev.abstain_reasons or "low_neff" in ev.abstain_reasons
